...
def letter_grades(highest: int) -> list[int]:
    ...
    """
    lowest: list[int] = [41]
    increment: int = round((highest - 41) / 4)

    for idx in range(1, 4):
        lowest.append(lowest[idx - 1] + increment)
...
def student_ranking(student_scores: list[int], student_names: list[str]) -> list[str]:
    ...
    """
    students_and_scores = list(zip(student_names, student_scores))
    ranked: list[str] = []
    for idx, student in enumerate(students_and_scores):
        rank = f"{idx + 1}. {student[0]}: {student[1]}"
        ranked.append(rank)
