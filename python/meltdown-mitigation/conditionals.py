...
def is_criticality_balanced(temperature: int | float, neutrons_emitted: int | float) -> bool:
    ...
    """
    neutrons_emitted_per_second = temperature * neutrons_emitted
    return temperature < 800\
        and neutrons_emitted > 500\
        and neutrons_emitted_per_second < 500000
...
def fail_safe(
    ...
    t_ninety = threshold * 0.9
    t_ten = threshold * 0.1

    if status < t_ninety:
        return 'LOW'
    if (threshold - t_ten) <= status <= (threshold + t_ten):
        return 'NORMAL'
    return 'DANGER'
